...
async function startSidecar(){
 ...
 const geminiKey=loadSecret('geminiApiKey');
 sidecarLastError='';
 sidecarProc=spawn(cmd,args,{
  cwd:sideDir(),windowsHide:true,
  env:{...process.env,GEMINI_API_KEY:geminiKey,GOOGLE_API_KEY:geminiKey,ATLAS_MODEL:process.env.ATLANEX_AGENT_MODEL||'gemini-2.5-flash'},
  stdio:['pipe','pipe','pipe']
 });
 sidecarProc.stdout.setEncoding('utf8');sidecarProc.stdout.on('data',handleSidecarStdout);
 sidecarProc.stderr.setEncoding('utf8');sidecarProc.stderr.on('data',x=>{const s=String(x||'').trim();if(s)sidecarLastError=s.slice(-4000)});
...
async function invokeHandler(handler,query={}){
 return await new Promise((resolve,reject)=>{
  let settled=false,statusCode=200;const headers={};
...
async function runSmoke(){
 ...
 try{
  ...
  console.log(JSON.stringify({ok:true,renderer:true,preload:true,transport:'electron-ipc+stdio'}));return 0
...
async function runAcceptance(){
 ...
 try{
  ...
  if(ag.status!==0||!payload.ok||!payload.crewai_runtime||!payload.gpt_researcher_runtime||!payload.google_genai_adapter||!payload.stdio_rpc||!payload.no_loopback_runtime||!payload.provider_objects){
   const detail=JSON.stringify({status:ag.status,payload,stderr:String(ag.stderr||'').slice(-8000),stdout:String(ag.stdout||'').slice(-8000)});
   throw new Error('agent deep acceptance failed '+detail)
  }
...
ipcMain.handle('atlas:ai-secret-set',async(_e,{provider,key})=>{if(!CLOUD_PROVIDERS.includes(provider))throw new Error('Unsupported provider');const v=String(key||'').trim();if(v&&v.length<20)throw new Error('Invalid cloud credential');const ok=saveSecret(provider+'ApiKey',v);if(provider==='gemini'){stopSidecar();if(fs.existsSync(agentExe())||fs.existsSync(devPython()))await startSidecar().catch(()=>false)}return ok});
ipcMain.handle('atlas:ai-secret-clear',async(_e,provider)=>{if(!CLOUD_PROVIDERS.includes(provider))throw new Error('Unsupported provider');const ok=saveSecret(provider+'ApiKey','');if(provider==='gemini'){stopSidecar();if(fs.existsSync(agentExe())||fs.existsSync(devPython()))await startSidecar().catch(()=>false)}return ok});
